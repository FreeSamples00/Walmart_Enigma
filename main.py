...
def encode(text_in, code):  # takes text and cipher, returns encoded text
    ...
    for u, letter in enumerate(text_in):
        ...
        offsets[0] = (
            offsets[0] + 1
        ) % rotor_len  # upping offsets to provide a unique per letter code
        if (u % 3) == 0:
...
def GUI_encode():  # call from the GUI button "Encode"
    ...
    if len(cipher_entry.get()) == 6:
        if file_loaded:  # file encoding
            ...
            len(text_entry.get("1.0", "end")) > 1
        ):  # 8 = 6 cipher digits + newline char + one input char
            text_exit.config(state="normal")
            text_exit.delete("1.0", "end")
            text_exit.insert(
                "1.0", encode(text_entry.get("1.0", "end-1c"), cipher_entry.get())
            )
            text_exit.config(state="disabled")
    else:
...
def GUI_decode():  # call from GUI button "Decode"
    ...
    if len(cipher_entry.get()) == 6:
        if file_loaded:  # file decoding
            ...
            text_exit.delete("1.0", "end")
            text_exit.insert(
                "1.0", decode(text_entry.get("1.0", "end-1c"), cipher_entry.get())
            )
            text_exit.config(state="disabled")
    else:
        messagebox.showerror("Error", "Cipher Needed")
...
def rand_cipher():  # call from the GUI button "Random"
    ...
    cipher_entry.insert(
        1, str(r.randint(100000, 999999))
    )  # magic numbers represent range of 6-digit numbers
...
encode_button = tk.Button(text="Encode", command=GUI_encode)
decode_button = tk.Button(text="Decode", command=GUI_decode)
random_button = tk.Button(frame1, text="Random", bg=ui_button_bg, command=rand_cipher)
